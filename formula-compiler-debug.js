...
class Compiler {
  constructor(context, options = {}) {
    ...
    this.maxRelationshipDepth = options.maxRelationshipDepth || 3; // Default max depth for multi-level relationships
  ...
  compileAggregateFunction(node) {
    ...
    if (parts.length > (maxDepth + 1) * 2) { // More conservative: allow for longer relationship names
      this.error(`Multi-level aggregate chain too deep (max ${maxDepth} levels): ${relationshipName}`, node.position);
    }
    
    // Parse the relationship name to identify the chain
    const relationshipChain = this.parseInverseRelationshipChain(relationshipName, originalRelationshipName, node.position);
    
    // Validate actual chain depth
    if (relationshipChain.length > maxDepth) {
      this.error(`Multi-level aggregate chain too deep (max ${maxDepth} levels): ${relationshipName}`, node.position);
    }
    
    // Build the aggregation context
    const aggregationContext = this.buildAggregationContext(relationshipChain, node.position);
    
    // Create sub-compilation context for aggregate expression
    const subCompiler = new Compiler(aggregationContext.subContext, { 
      maxRelationshipDepth: this.maxRelationshipDepth,
      maxInverseAggregateDepth: this.maxInverseAggregateDepth 
    });
    subCompiler.compilationContext = aggregationContext.compilationContext;
    ...
    const aggSemanticId = this.generateSemanticId(relationshipChain.length > 1 ? 'multi_aggregate' : 'aggregate', semanticDetails);
    ...
    // Only add multiLevel property for multi-level aggregates
    if (relationshipChain.length > 1) {
      aggregateIntent.multiLevel = true;
    }
  ...
  /**
   * Parse inverse relationship chain (unified for all levels)
   * @param {string} relationshipName - Relationship name to parse (lowercase)
   * @param {string} originalRelationshipName - Original case relationship name for error messages
   * @param {number} position - Position for error reporting
  ...
  parseInverseRelationshipChain(relationshipName, originalRelationshipName, position) {
    console.log('🔍 DEBUG: Parsing relationship chain:', originalRelationshipName);
    console.log('🔍 DEBUG: Available inverse relationships:', Object.keys(this.context.inverseRelationshipInfo || {}));
  ...
  /**
   * Build aggregation context (unified for all levels)
   * @param {Array<Object>} relationshipChain - Chain of relationships
   * @param {number} position - Position for error reporting
   * @returns {Object} Context and compilation info
   */
  buildAggregationContext(relationshipChain, position) {
    if (relationshipChain.length === 1) {
      // Single-level aggregation - use existing logic
      const relInfo = this.context.inverseRelationshipInfo[relationshipChain[0].originalName];
      
      // Build sub-context from the nested structure
      const subTableInfos = [
        {
      ...
      if (relInfo.relationshipInfo) {
        for (const [relName, subRelInfo] of Object.entries(relInfo.relationshipInfo)) {
          subRelationshipInfos.push({
            name: relName,
            fromTable: relInfo.tableName,
            toTable: subRelInfo.tableName || relName,
            joinColumn: subRelInfo.joinColumn
          });
          
          // Add target table info if not already present
          if (!subTableInfos.find(t => t.tableName === (subRelInfo.tableName || relName))) {
            subTableInfos.push({
              tableName: subRelInfo.tableName || relName,
      ...
      return {
        ...
        compilationContext: `agg:${this.tableName}→${relInfo.tableName}[${relInfo.joinColumn}]`,
        finalTable: relInfo.tableName,
        relationshipChain: relationshipChain
      };
    } else {
      // Multi-level aggregation - build chain context
      let currentTable = this.tableName;
      let compilationPath = [];
      
      for (let i = 0; i < relationshipChain.length; i++) {
        const chainStep = relationshipChain[i];
        const { inverseTable, joinField } = chainStep;
      ...
      if (!finalTableInfo) {
        finalTableInfo = {
          ...
          columnList: {}
             ...
             // Build the tableInfos array with the final table and any related tables
       const subTableInfos = [finalTableInfo];
       
               // Add target tables for the relationships
        for (const relInfo of finalRelationshipInfos) {
          if (!subTableInfos.find(t => t.tableName === relInfo.toTable)) {
            // Try to find the target table info from multiple sources
            let targetTableInfo = null;
            
            // First, try to find from the last relationship's nested info
            if (relationshipChain.length > 0) {
              const lastRelName = relationshipChain[relationshipChain.length - 1].originalName;
              const lastRelInfo = this.context.inverseRelationshipInfo[lastRelName];
              
              if (lastRelInfo && lastRelInfo.relationshipInfo && lastRelInfo.relationshipInfo[relInfo.name]) {
                const nestedRelInfo = lastRelInfo.relationshipInfo[relInfo.name];
                targetTableInfo = {
                  tableName: relInfo.toTable,
                  columnList: nestedRelInfo.columnList || {}
                };
              }
            }
            
            // Second, try to find from the main context's tableInfos
            if (!targetTableInfo && this.context.tableInfos) {
              const mainTableInfo = this.context.tableInfos.find(t => t.tableName === relInfo.toTable);
              if (mainTableInfo) {
                targetTableInfo = {
                  tableName: relInfo.toTable,
                  columnList: mainTableInfo.columnList || {}
                };
              }
            }
            
            // Fallback: create minimal table info
            if (!targetTableInfo) {
              targetTableInfo = {
                tableName: relInfo.toTable,
                columnList: {}
