...
import { FUNCTION_METADATA, FUNCTIONS, CATEGORIES } from './function-metadata.js';
...
export class FormulaLanguageServer {
  ...
  getCompletions(text, position, tableName = null) {
    try {
      ...
      if (context.expectingFunction || context.expectingIdentifier) {
        completions.push(...this.getFunctionCompletions(context.prefix));
      }

      // Column completions (for current table)
      if (context.expectingIdentifier && tableName && this.schema) {
        completions.push(...this.getColumnCompletions(tableName, context.prefix));
        
        // Relationship completions
        completions.push(...this.getRelationshipCompletions(tableName, context.prefix));
      }
      ...
      if (context.expectingOperator) {
        completions.push(...this.getOperatorCompletions(context.prefix));
      }
  ...
  /**
   * Get column completions for a table
   */
  getColumnCompletions(tableName, prefix = '') {
  ...
  /**
   * Get relationship completions
   */
  getRelationshipCompletions(tableName, prefix = '') {
    if (!this.schema || !this.schema[tableName]) {
      return [];
    }
...
export { CompletionItemKind, DiagnosticSeverity };