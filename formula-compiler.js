...
class Compiler {
  ...
  compileFunction(node) {
    ...
    if (funcName === 'DATE') {
      return this.compileDateFunction(node);
    }
    
    if (funcName === 'DATEDIF') {
      return this.compileDateDifFunction(node);
    }
    
    if (funcName === 'NULLVALUE') {
      return this.compileNullValueFunction(node);
    }
    
    if (['AND', 'OR'].includes(funcName)) {
      return this.compileVariadicLogicalFunction(node, funcName);
    }
    
    // Handle all other functions using metadata
    return this.compileStandardFunction(node, funcName);
  }

  /**
   * Compile standard functions using metadata for validation and type checking
   */
  compileStandardFunction(node, funcName) {
    ...
    const expectedArgCount = metadata.arguments.length;
    if (node.args.length !== expectedArgCount) {
      if (expectedArgCount === 0) {
        this.error(`${funcName}() takes no arguments`, node.position);
      } else if (expectedArgCount === 1) {
    ...
    // Compile and validate each argument
    const compiledArgs = [];
    const allDependentJoins = [];
    const childIds = [];
    
    for (let i = 0; i < node.args.length; i++) {
      const compiledArg = this.compile(node.args[i]);
      const expectedType = metadata.arguments[i].type;
      const actualType = compiledArg.returnType;
      
      // Type validation (skip for 'expression' type which accepts any)
      if (expectedType !== 'expression' && actualType !== expectedType) {
        const argName = metadata.arguments[i].name;
        this.error(`${funcName}() ${argName} must be ${expectedType}, got ${actualType}`, node.position);
      }
      
      compiledArgs.push(compiledArg);
      allDependentJoins.push(...compiledArg.dependentJoins);
      childIds.push(compiledArg.semanticId);
    }
    
    // Determine return type
    let returnType = metadata.returnType;
    if (returnType === 'varies') {
      // For functions with dynamic return types, use the first argument's type
      returnType = compiledArgs[0].returnType;
    }
    
    // Special value handling for certain functions
    let value = { name: funcName };
    
    return {
      type: 'FUNCTION_CALL',
      semanticId: this.generateSemanticId('function', funcName, childIds),
      dependentJoins: allDependentJoins,
  ...
  /**
   * Compile DATE function with special string literal validation
   */
  compileDateFunction(node) {
  ...
  /**
   * Compile DATEDIF function with special unit validation
   */
  compileDateDifFunction(node) {
  ...
  /**
   * Compile NULLVALUE function with special type matching logic
   */
  compileNullValueFunction(node) {
  ...
  /**
   * Compile variadic logical functions (AND, OR) with special argument validation
   */
  compileVariadicLogicalFunction(node, funcName) {
    if (node.args.length < 2) {
    ...
    for (let i = 0; i < node.args.length; i++) {
      ...
      if (arg.returnType !== 'boolean') {
        this.error(`${funcName}() argument ${i + 1} must be boolean, got ${arg.returnType}`, node.position);
      }
      args.push(arg);
      dependentJoins.push(...arg.dependentJoins);
      childIds.push(arg.semanticId);
    }
