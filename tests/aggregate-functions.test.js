...
test('STRING_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.string_agg_result_1');
  assertEqual(result.aggregateJoins.length, 1);
  assertEqual(result.aggregateJoins[0].alias, 'agg1');
});
...
test('STRING_AGG_DISTINCT basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.string_agg_distinct_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('SUM_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.sum_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('COUNT_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.count_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('AVG_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.avg_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('MIN_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.min_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('MAX_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.max_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('AND_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.bool_and_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});
...
test('OR_AGG basic usage', () => {
  ...
  assertEqual(result.expression, 'agg1.bool_or_result_1');
  assertEqual(result.aggregateJoins.length, 1);
});

// Multiple aggregates on same relationship (should share subquery)
test('Multiple aggregates on same relationship', () => {
  const result = evaluateFormula('SUM_AGG(rep_links_submission, commission_percentage) + COUNT_AGG(rep_links_submission, rep)', relationshipContext);
  assertEqual(result.aggregateJoins.length, 1);
  assertEqual(result.aggregateJoins[0].functions.size, 2);
});
...
test('Multiple aggregates on different relationships', () => {
  ...
  assertEqual(result.aggregateJoins.length, 2);
});
...
test('Aggregate with nested relationship', () => {
  const result = evaluateFormula('STRING_AGG(rep_links_submission, rep_rel.name, ",")', relationshipContext);
  assertEqual(result.expression, 'agg1.string_agg_result_1');
  assertEqual(result.aggregateJoins[0].requiredJoins.length, 1);
});
